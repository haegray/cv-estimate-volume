"""
Test triangulation-based reconstruction on cube dataset.
This should produce a cube shape instead of a cone.
"""
...
# Test with cube images - use a smaller subset for quick testing
three_images = [
...
# Import necessary functions from volume-test.py
import importlib.util
spec = importlib.util.spec_from_file_location("volume_test", "./volume-test.py")
volume_test = importlib.util.module_from_spec(spec)

# We'll run a simplified version inline to test triangulation
...
def get_point_matches(img1, img2):
    ...
    if mask is None:
        # Fundamental matrix computation failed
        return np.array([]), pts1, pts2, None
...
if len(all_points) > 0:
    ...
    try:
        ...
        ax.set_ylabel('Y')
        ax.set_zlabel('Z')
        ax.set_title('Cube Reconstruction using Triangulation')
